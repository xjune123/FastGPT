...
const InputDataModal = ({
  ...
  const onSelectFile = useCallback(async (e: File[]) => {
    ...
    try {
      ...
      form.append('bucketName', 'public');
      const data = await fetch(
        'https://pkdc.pooksh.com:8080/midjourney/v1/0/mj/new/file/upload/multipart',
        {
          method: 'POST',
          body: form
        }
      ...
      const str = `${getValues('q')}\n![image](${data})`;
      setValue('q', str);
    } catch (err: any) {
      toast({
        title: typeof err === 'string' ? err : '选择异常',
        status: 'warning'
      });
    }
  }, []);
